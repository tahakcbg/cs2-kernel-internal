...
namespace detail {
    ...
    static constexpr rgb k_success_light{ 100, 235, 155 };
    static constexpr rgb k_success_dark{ 35, 165, 85 };
...
bool console::initialize( std::string_view title )
{
    ...
    {
        csbi.ColorTable[0] = RGB( 12, 14, 20 );
    ...
    if ( hwnd )
    {
        ...
        SMALL_RECT window_size{ 0, 0, 95, 29 };
        ::SetConsoleWindowInfo( this->m_handle, TRUE, &window_size );

        COORD buffer_size{ 96, 9999 };
...
void console::emit( level lvl, const std::string& message ) const
{
    ...
    const auto label = detail::gradient_text( "vigil", grad_from, grad_to );
    ...
    std::printf( "  %s%s %s%s %s-%s %s%s%s\n",
        detail::k_dim, bracket_col.c_str( ), label.c_str( ), bracket_col.c_str( ),
        detail::k_dim, detail::k_reset, body_col.c_str( ), message.c_str( ), detail::k_reset );
}

void console::wait_and_exit( ) const
{
    const auto err_col = std::format( "\033[38;2;{};{};{}m",
        detail::k_error_light.r, detail::k_error_light.g, detail::k_error_light.b );
